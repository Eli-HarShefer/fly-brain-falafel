...
const SCENES = {
  ...
  slicing: {
    title: 'ככה מיפו אותו',
    sub: 'חתכו אותו ל<em>אלפי פרוסות</em> וצילמו כל אחת. יצאו <em>21 מיליון</em> תמונות.',
  ...
  slice: {
    title: 'זאת פרוסה אחת',
    sub: 'רקמת מוח אמיתית. <em>גוגל</em> בנתה רשת נוירונים שעקבה אחרי כל סיב לבד.',
  ...
  live: {
    ...
    layout: 'live',
    frames: [
      { kind: 'stand', tag: 'הדוכן', full: true },
...
async function main() {
  ...
  function build(kind, canvas, opt = {}) {
    ...
    if (kind === 'stand') {
      ...
      // clean strips the order board, the queue and the effects. Right for a
      // small reference panel, wrong for the opener, where the customers
      // waiting and the order to fill are the thing that reads as a game.
      s.clean = !opt.full;
      s.resize();
